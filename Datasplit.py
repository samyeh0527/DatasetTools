...
def cpimages(path,savepath,labels):
    ...
    AllimgList = str(AllimgList).replace('.jpg','')
    AllimgList = str(AllimgList).replace("'",'')
    AllimgList = str(AllimgList).replace('[','')
    AllimgList = AllimgList.replace(']','')
    ...
    for _ in tqdm(range(len(pathList))):
        ...
        filelist =str(filelist).replace('.txt','')
